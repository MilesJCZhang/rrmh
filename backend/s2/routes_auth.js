...
router.post('/wechat-login', async (req, res) => {
  ...
  try {
    ...
    // DEV: 模拟登录（code 以 mock 开头时跳过微信验证）
    if (process.env.NODE_ENV === 'development' && String(code).startsWith('mock')) {
      console.log('[auth] DEV mock login, code:', code);
      const mockUser = {
        id: parseInt(String(code).replace('mock', '')) || 1,
        openid: 'dev_openid_' + code,
        nickname: '开发用户',
        avatar: '',
        role: 'partner_matchmaker',
        recommendCode: 'DEV0001',
      };
      // 确保用户存在
      let user = db.prepare('SELECT * FROM users WHERE openid = ?').get(mockUser.openid);
      if (!user) {
        const info = db.prepare('INSERT INTO users (openid, nickname, role, recommend_code, status) VALUES (?, ?, ?, ?, 1)').run(
          mockUser.openid, mockUser.nickname, mockUser.role, mockUser.recommendCode
        );
        mockUser.id = info.lastInsertRowid;
        user = mockUser;
      }
      const jwt = require('jsonwebtoken');
      ...
      return res.json({
        code: 0,
        data: { token, isNewUser: false, users: { id: user.id, nickname: user.nickname, avatar: user.avatar, role: user.role, recommendCode: user.recommendCode } },
        message: 'DEV模式登录成功',
      });
    }
    ...
    // 解析 referrer_id：优先使用传入的 referrer_id，否则通过 referral_code 反查
    let referrerIdNum = referrer_id ? parseInt(referrer_id) : null;
    if (!referrerIdNum && referral_code) {
      // 通过推荐码反查 referrer_id（支持小程序码场景：scene 直接带推荐码而非数字ID）
      const codeRow = db.prepare(
        'SELECT referrer_id FROM referral_codes WHERE code = ? AND status = \'active\' LIMIT 1'
      ).get(String(referral_code).toUpperCase());
      if (codeRow && codeRow.referrer_id) {
        referrerIdNum = codeRow.referrer_id;
        console.log(`[wechat-login] 通过推荐码 ${referral_code} 反查到 referrer_id: ${referrerIdNum}`);
      }
    }

    if (!user) {
      // 3. 新用户，插入记录（自动绑定推荐人）
      // 同时写入 parent_id 和 referrer_id（兼容支付校验读取 referrer_id 的逻辑）
      const info = db.prepare(`
        INSERT INTO users (openid, parent_id, referrer_id, created_at)
        VALUES (?, ?, ?, datetime('now'))
      `).run(openid, referrerIdNum, referrerIdNum);
      ...
      // 已有用户：若 parent_id 为 null 且本次带来了推荐人信息，补填（永久锁定，只允许从 null → 有值）
      const hasReferrerInput = referrer_id || referral_code;
      if (user.parent_id === null && hasReferrerInput) {
        // 如果没有传入 referrer_id，则通过 referral_code 反查
        let fillReferrerId = referrer_id ? parseInt(referrer_id) : null;
        if (!fillReferrerId && referral_code) {
          const codeRow = db.prepare(
            'SELECT referrer_id FROM referral_codes WHERE code = ? AND status = \'active\' LIMIT 1'
          ).get(String(referral_code).toUpperCase());
          if (codeRow && codeRow.referrer_id) {
            fillReferrerId = codeRow.referrer_id;
          }
        }
        if (fillReferrerId) {
          // 同时更新 parent_id 和 referrer_id（确保支付校验能读取到推荐人）
          db.prepare(`UPDATE users SET parent_id = ?, referrer_id = ?, updated_at = datetime('now') WHERE id = ?`)
            .run(fillReferrerId, fillReferrerId, user.id);
          console.log(`[wechat-login] 补填 parent_id 和 referrer_id: user ${user.id} → ${fillReferrerId}`);
          user.parent_id = fillReferrerId;
          ...
          try {
            ...
            if (!existing) {
              db.prepare(`
                INSERT INTO user_referrals (user_id, referrer_id, referral_code, bind_time, is_locked)
                VALUES (?, ?, ?, datetime('now'), 1)
              `).run(user.id, fillReferrerId, referral_code || null);
              console.log(`[wechat-login] 写入 user_referrals: user ${user.id} → referrer ${fillReferrerId}`);
            }
          } catch (e) {
            console.error('[wechat-login] 写入 user_referrals 失败:', e.message);
          }
...
router.post('/bind-phone', async (req, res) => {
  ...
  try {
    ...
    const userId = payload.userId || payload.userId || payload.uid;
    const { code } = req.body;
...
router.get('/userinfo', (req, res) => {
  ...
  try {
    ...
    // 查询用户信息
    const user = db.prepare('SELECT * FROM users WHERE id = ?').get(payload.userId || payload.uid);
...
router.post('/refresh', (req, res) => {
  ...
  try {
    ...
    const newToken = jwt.sign(
      { userId: payload.userId || payload.uid, openid: payload.openid, role: user.role },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
