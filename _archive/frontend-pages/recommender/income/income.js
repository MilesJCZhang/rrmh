...
Page({
  ...
  onWithdraw: function() {
    ...
    const userInfo = authService.getUserInfo();
    const defaultAccount = userInfo.wechatAccount || '';

    wx.showModal({
      title: '提现申请',
      content: `可提现 ¥${withdrawable}\n${feeInfo}\n实际到账 ¥${actual}\n\n收款方式：${defaultAccount ? '微信(' + defaultAccount + ')' : '将在付款前确认'}`,
      success: (res) => {
        if (res.confirm) {
          this.submitWithdraw(defaultAccount, '微信', userInfo.nickname);
        }
      }
    });
  },
  ...
  // 提交提现申请（真实API版）
  submitWithdraw: function(bankAccount, bankName, accountHolder) {
    wx.showLoading({ title: '提交中...' });

    const amount = this.data.withdrawable;
    const request = require('../../../utils/request');
    const userInfo = require('../../../services/auth.service').getUserInfo();

    // 默认使用用户微信账号
    const account = bankAccount || userInfo.wechatAccount || '';
    const holder = accountHolder || userInfo.nickname || '';
    const bank = bankName || '微信';
    ...
    request.post('/api/withdrawals', {
      ...
      if (res && res.code === 0) {
        // 更新本地数据
        this.setData({
          withdrawn: this.data.withdrawn + amount,
          withdrawable: 0,
        });
        wx.showModal({
          title: '提现申请已提交',
          content: '提现金额 ¥' + amount.toFixed(2) + '，实际到账 ¥' + (res.data?.actualAmount || (amount * 0.87).toFixed(2)) + '（含13%服务费），请等待管理员审核。',
          showCancel: false
        });
      } else {
        wx.showModal({
