...
Page({
  ...
  loadRecommenderData: function() {
    ...
    Promise.all([
      ...
      referralService.getWorkbenchStats().catch(err => {
        console.warn('[recommender] 工作台统计获取失败', err);
        return null;
      })
    ]).then(([insightRes, summaryRes, recordsRes, wbStatsRes]) => {
      wx.hideLoading();
  ...
  onSearchConfirm() {
    ...
    if (expandedType) { this.setData({ detailPage: 1, detailList: [] }); this._loadDetailList(expandedType, searchKeyword); }
  },
  onClearSearch() {
    this.setData({ searchKeyword: '' });
    const { expandedType } = this.data;
    if (expandedType) { this.setData({ detailPage: 1, detailList: [] }); this._loadDetailList(expandedType, ''); }
  },
  onToggleDetail(e) {
  ...
  async _loadDetailList(type, keyword = '') {
    ...
    try {
      const result = await referralService.getWorkbenchDetail({ type, page: detailPage, page_size: 10, keyword });
      const newList = result?.data?.list || result?.list || [];
      const total = result?.data?.total || result?.total || 0;
      this.setData({ detailList: detailPage === 1 ? newList : [...detailList, ...newList], hasMore: (detailPage === 1 ? newList : [...detailList, ...newList]).length < total });
    } catch (err) { wx.showToast({ title: '加载明细失败', icon: 'none' }); console.error('[recommender] 明细加载失败', err); }
  },
  async onRefresh() {
    this.setData({ isRefreshing: true });
    await this.loadRecommenderData();
