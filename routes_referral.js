...
router.post('/verify', optionalAuth, (req, res) => {
  try {
    ...
    const codeRow = db.prepare(`
      SELECT rc.id, rc.code, rc.code_type, rc.referrer_id, rc.referrer_name,
             rc.status, rc.use_count, rc.max_uses, rc.expires_at,
             u.nickname as referrer_nickname, u.role as referrer_role
    ...
    if (!codeRow) {
      return res.json({ code: 0, data: { valid: false, message: '推荐码不存在' } });
    }
    if (codeRow.status !== 'active') {
      return res.json({ code: 0, data: { valid: false, message: '推荐码已失效' } });
    }
    if (codeRow.expires_at && new Date(codeRow.expires_at) < new Date()) {
      return res.json({ code: 0, data: { valid: false, message: '推荐码已过期' } });
    }
    if (codeRow.max_uses > 0 && codeRow.use_count >= codeRow.max_uses) {
      return res.json({ code: 0, data: { valid: false, message: '推荐码使用次数已达上限' } });
    }
    if (!codeRow.referrer_id) {
      return res.json({ code: 0, data: { valid: false, message: '推荐码未关联推荐官，请联系管理员' } });
    }
    ...
    res.json({
      ...
      data: {
        ...
        referrer_name: codeRow.referrer_nickname || codeRow.referrer_name || '推荐官',
        message: '推荐码有效',
      },
    });

  } catch (err) {
    logger.error('[referral] verify error:', err);
    res.status(500).json({ code: -1, message: '验证失败' });
  }
});
  ...
  try {
    ...
    switch (type) {
      case 'partner_matchmaker':
        ...
        `;
        listSql = `
          SELECT DISTINCT ur.user_id as id, u.nickname, u.avatar, u.role, u.created_at,
