...
/**
 * 生成唯一推荐码（SQLite 版本）
 * 各角色码池严格隔离：referral_codes 表按 code_type 分离，users 表按 role 分离
...
function generateUniqueCode(db, role) {
  ...
  // 获取该角色在 referral_codes 表中的 code_type（public_matchmaker → public_welfare）
  const codeType = ROLE_TO_CODE_TYPE[role] || role;
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    // referral_codes 表：同 code_type 下唯一（不同角色码池隔离）
    const existsInCodes = db.prepare(
      'SELECT id FROM referral_codes WHERE code = ? AND code_type = ?'
    ).get(code, codeType);
    // users 表：同 role 下唯一（确保该角色用户不重复）
    const existsInUsers = db.prepare(
      'SELECT id FROM users WHERE referral_code = ? AND role = ?'
    ).get(code, role);
...
router.post('/public-matchmaker', requireAuth, (req, res) => {
  try {
    ...
    // 检查现有推荐码前缀是否匹配公益推荐官（GYRG开头），不匹配则需重新生成
    const needsNewCode = !code || !code.startsWith('GYRG');
    ...
    if (needsNewCode) {
      ...
      const applyTransaction = db.transaction(() => {
        ...
        // 记录申请（写入 apply_records 表）
        db.prepare(`
          INSERT OR REPLACE INTO apply_records (user_id, target_role, status, created_at, updated_at)
          VALUES (?, 'public_matchmaker', 'approved', datetime('now'), datetime('now'))
        `).run(userId);
      });

      applyTransaction();
    } else {
      // 已有 GYRG 前缀推荐码，只需更新角色
      db.prepare(`
...
router.post('/partner-matchmaker', requireAuth, (req, res) => {
  try {
    ...
    const user = db.prepare('SELECT id, role, referrer_id FROM users WHERE id = ?').get(userId);
    if (!user) {
      return res.json({ code: -1, message: '用户不存在' });
    }
    ...
    // 检查用户是否有推荐人（联创推荐官必须有推荐人或系统授权）
    // users.referrer_id：推荐码绑定后写入
    // user_referrals：显式的绑定记录（is_locked=1 表示已锁定）
    const userReferral = db.prepare(
      'SELECT referrer_id FROM user_referrals WHERE user_id = ? AND is_locked = 1'
    ).get(userId);
    const effectiveReferrerId = user.referrer_id || (userReferral ? userReferral.referrer_id : null);

    // 检查推荐人是否系统授权：查推荐人的推荐码是否为 SYSTEM 标记
    let isSystemAuthorized = false;
    if (effectiveReferrerId) {
      const refCode = db.prepare(
        "SELECT code FROM referral_codes WHERE referrer_id = ? AND code_type = 'system' LIMIT 1"
      ).get(effectiveReferrerId);
      if (refCode) isSystemAuthorized = true;
    }
    // 也检查用户自身是否直接绑定 SYSTEM 推荐码
    if (!isSystemAuthorized && userReferral) {
      try {
        const directSys = db.prepare(
          "SELECT id FROM user_referrals WHERE user_id = ? AND is_locked = 1 AND referred_by_code = 'SYSTEM' LIMIT 1"
        ).get(userId);
        if (directSys) isSystemAuthorized = true;
      } catch (e) { /* referred_by_code 列可能不存在，忽略 */ }
    }

    if (!effectiveReferrerId && !isSystemAuthorized) {
      return res.json({ code: -1, message: '成为联创推荐官需要先通过推荐码绑定一位推荐人' });
    }
