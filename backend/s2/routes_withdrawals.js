...
// ===================================================================
// 以下路由专为 /api/admin/withdrawals 挂载点设计（无 /admin/ 前缀）
// 前端 withdrawal.service.ts 调用:
//   GET  /api/admin/withdrawals（带 page/limit/status 参数）
//   PUT  /api/admin/withdrawals/:id/approve
//   PUT  /api/admin/withdrawals/:id/mark-paid
// ===================================================================

/**
 * 9. 管理员提现列表（适配前端 GET /api/admin/withdrawals）
 */
router.get('/', (req, res) => {
  const db = req.app.get('db');
  const { page = 1, limit = 10, status } = req.query;
  ...
  try {
    ...
    if (status !== undefined && status !== '') {
      // 前端用数字 0/1/2/3, 后端用字符串
      const statusMap = { '0': 'pending', '1': 'approved', '2': 'paid', '3': 'rejected' };
    ...
    const countStmt = db.prepare(`SELECT COUNT(*) as total FROM withdrawals w ${whereClause}`);
    const { total } = countStmt.get(...params);
    ...
    // 转为前端 Withdrawal 接口格式（camelCase + 数字状态）
    const statusReverseMap = { pending: 0, approved: 1, paid: 2, rejected: 3 };
    const list = items.map(w => ({
      ...
      status: statusReverseMap[w.status] ?? 0,
      accountInfo: `${w.bank_name || ''} ${w.bank_account || ''}`.trim(),
      remark: w.reject_reason || '',
    ...
    console.error('[withdrawals] admin list error:', error);
    res.json({ list: [], total: 0 });
  }
});

/**
 * 10. 管理员审核提现（适配前端 PUT /api/admin/withdrawals/:id/approve）
...
router.put('/:id/approve', (req, res) => {
  ...
  try {
    ...
    if (withdrawal.status !== 'pending') return res.status(400).json({ code: -1, message: '只能审核待审核的提现申请' });

    const doApprove = db.transaction(() => {
      db.prepare('UPDATE withdrawals SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(status, id);
      ...
      if (status === 'approved') {
        ...
        db.prepare('UPDATE withdrawals SET processed_at = CURRENT_TIMESTAMP WHERE id = ?').run(id);
      }
    });

    doApprove();
    res.json({ code: 0, message: status === 'approved' ? '审核通过' : '已拒绝' });
  } catch (error) {
...
/**
 * 11. 管理员标记已打款（适配前端 PUT /api/admin/withdrawals/:id/mark-paid）
 */
router.put('/:id/mark-paid', (req, res) => {
  ...
  try {
    const withdrawal = db.prepare('SELECT * FROM withdrawals WHERE id = ?').get(id);
    if (!withdrawal) return res.status(404).json({ code: -1, message: '提现记录不存在' });
    if (withdrawal.status !== 'approved') return res.status(400).json({ code: -1, message: '只能标记已审核通过的提现' });

    db.prepare("UPDATE withdrawals SET status = 'paid', processed_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(id);
    db.prepare(
      `UPDATE commissions SET status = 'paid', updated_at = datetime('now')
       WHERE withdrawal_id = ? AND status = 'withdrawn'`
    ).run(id);

    res.json({ code: 0, message: '已标记为打款完成' });
  } catch (error) {
    console.error('[withdrawals] mark-paid error:', error);
    res.status(500).json({ code: -1, message: '标记打款失败' });
  }
});
