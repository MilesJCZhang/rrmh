...
const APPLY_CONFIG = {
  ...
  franchisee: {
    ...
    fields: ['companyName', 'contactName', 'gender', 'phone', 'wechat', 'city', 'region', 'experience', 'plan', 'bankAccount'],
    fieldLabels: {
      companyName: '公司/个体名称',
      contactName: '负责人姓名',
      gender: '性别',
  ...
  community: {
    ...
    // 单位简介（选填）、收款银行账户
    fields: ['orgType', 'orgName', 'address', 'contactPerson', 'gender', 'phone', 'wechat', 'orgDescription', 'bankAccount', 'bankName'],
    fieldLabels: {
      orgType: '单位类型',
      orgName: '单位名称',
      address: '单位地址',
...
Page({
  ...
  onGenderPick(e) {
    ...
    const genderVal = this.data.genderOptions[idx];
    this.setData({
      genderIndex: idx,
      'formData.gender': genderVal,
    });
    // 立即缓存性别到 authService，不依赖提交结果
    const dbGender = genderVal === '男' ? 'male' : (genderVal === '女' ? 'female' : '');
    if (dbGender) {
      const info = authService.getUserInfo() || {};
      info.gender = dbGender;
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (formData.gender) {
        const info = authService.getUserInfo() || {};
        info.gender = formData.gender === '男' ? 'male' : 'female';
        authService.setUserInfo(info);
      }
  ...
  _loadPartnerName(referrerId) {
    ...
    // 静默加载推荐人姓名，失败时使用默认名称
    request({
      url: `/v1/user/${referrerId}/avatar-info`,
      withToken: false,
    }).then((data) => {
