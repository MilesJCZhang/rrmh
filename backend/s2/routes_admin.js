...
router.get('/dashboard/stats', requireAuth, requireAdmin, (req, res) => {
  ...
  try {
    ...
    let totalRevenue = 0;
    try {
      totalRevenue = db.prepare('SELECT COALESCE(SUM(total_fee), 0) as total FROM orders WHERE status = "paid"').get().total;
    } catch (e) {
      // orders表可能不存在，忽略错误
    }
    ...
    // 7. 付费会员数（支付了 single_registration 建档费的用户）
    let paidMembers = 0;
    try {
      paidMembers = db.prepare(`
    ...
    let referrerCount = 0;
    try {
      referrerCount = db.prepare(`
        SELECT COUNT(*) as count FROM users
        WHERE role IN ('partner_matchmaker','professional_recommender','city_franchisee','community_station','public_matchmaker','creator','professional','city_partner','public_welfare')
      `).get().count;
    } catch (e) {
      // 忽略错误
    }
