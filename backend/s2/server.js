...
// 1.0 文件上传路由（放在认证之前，上传操作携带 token 鉴权）
try {
  app.use('/v1/upload', require('./routes_upload'));
  console.log('[server] routes_upload 加载成功');
} catch (e) {
  console.log('[server] routes_upload 加载失败:', e.message);
}

// 1.0.1 内容安全检测路由
try {
  app.use('/v1/moderation', require('./routes_moderation'));
  console.log('[server] routes_moderation 加载成功');
} catch (e) {
  console.log('[server] routes_moderation 加载失败:', e.message);
}
...
// 3. 用户路由
app.use('/v1/user', require('./routes_user'));
...
// ===== P0 修复：新注册路由（2026-06-17）=====
// 管理后台-实名认证审核
try {
  app.use('/api/admin/verifications', requireAuth, requireAdmin, require('./routes_admin_verify'));
  console.log('[server] routes_admin_verify 加载成功');
} catch (e) {
  console.log('[server] routes_admin_verify 加载失败:', e.message);
}
// 管理后台-收益/支付/财务统计
try {
  app.use('/api/admin', requireAuth, requireAdmin, require('./routes_admin_earnings'));
  console.log('[server] routes_admin_earnings 加载成功');
} catch (e) {
  console.log('[server] routes_admin_earnings 加载失败:', e.message);
}
