...
router.get('/users', (req, res) => {
  ...
  try {
    ...
    const list = db.prepare(`
      SELECT id, nickname, avatar, phone, gender, age, education,
             occupation, income, city AS location,
             profile_score, score_tier,
             face_auth_status, role, referral_code, created_at
      FROM users
      ${whereClause}
...
router.get('/users/:id', (req, res) => {
  ...
  try {
    const user = db.prepare(`
      SELECT id, nickname, avatar, phone, gender, age, birth_date,
             occupation, income, city AS location,
             profile_score, score_tier,
             face_auth_status, role, referral_code, created_at,
