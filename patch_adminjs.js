/**
 * 补丁脚本：为 dist/routes/admin.js 添加沙龙配置路由
 * 使用方法：node patch_adminjs.js /path/to/dist/routes/admin.js
 */

const fs = require('fs');
const path = require('path');

const targetFile = process.argv[2] || path.join(__dirname, 'dist/routes/admin.js');
...
// 要插入的路由代码（在 exports.default = router; 之前）
const salonConfigRoutes = `
// ==================== 沙龙配置管理 ====================
// 获取所有沙龙配置
router.get('/salon-configs', async (req, res) => {
  try {
    const configs = await prisma_1.default.$queryRaw\`SELECT * FROM salon_configs ORDER BY id\`;
    ...
    (0, response_1.success)(res, parsedConfigs, '获取成功');
  } catch (error) {
    console.error('[salon-config] 获取配置列表失败:', error);
    (0, response_1.error)(res, '获取配置列表失败', 500);
  }
});

// 获取单个沙龙配置
router.get('/salon-configs/:type', async (req, res) => {
  try {
    const { type } = req.params;
    const configs = await prisma_1.default.$queryRaw\`SELECT * FROM salon_configs WHERE type = \${type}\`;
    if (!configs || configs.length === 0) {
      return (0, response_1.error)(res, '配置不存在', 404);
    }
    ...
    (0, response_1.success)(res, parsedConfig, '获取成功');
  } catch (error) {
    console.error('[salon-config] 获取配置失败:', error);
    (0, response_1.error)(res, '获取配置失败', 500);
  }
});

// 创建沙龙配置
router.post('/salon-configs', async (req, res) => {
  try {
    const {
      type,
      name,
    ...
    const newConfig = await prisma_1.default.$queryRaw\`SELECT * FROM salon_configs WHERE type = \${type}\`;
    (0, response_1.success)(res, newConfig[0], '配置创建成功');
  } catch (error) {
    console.error('[salon-config] 创建配置失败:', error);
    (0, response_1.error)(res, '创建配置失败', 500);
  }
});

// 更新沙龙配置
router.put('/salon-configs/:type', async (req, res) => {
  try {
    const { type } = req.params;
    const {
      name,
    ...
    const updatedConfig = await prisma_1.default.$queryRaw\`SELECT * FROM salon_configs WHERE type = \${type}\`;
    (0, response_1.success)(res, updatedConfig[0], '配置更新成功');
  } catch (error) {
    console.error('[salon-config] 更新配置失败:', error);
    (0, response_1.error)(res, '更新配置失败', 500);
  }
});

// 删除沙龙配置
router.delete('/salon-configs/:type', async (req, res) => {
  try {
    const { type } = req.params;
    const result = await prisma_1.default.$executeRaw\`DELETE FROM salon_configs WHERE type = \${type}\`;
    (0, response_1.success)(res, null, '配置删除成功');
  } catch (error) {
    console.error('[salon-config] 删除配置失败:', error);
    (0, response_1.error)(res, '删除配置失败', 500);
  }
});

// 启用/禁用沙龙配置
router.patch('/salon-configs/:type/status', async (req, res) => {
  try {
    const { type } = req.params;
    ...
    const updatedConfig = await prisma_1.default.$queryRaw\`SELECT * FROM salon_configs WHERE type = \${type}\`;
    (0, response_1.success)(res, updatedConfig[0], \`配置已\${status === 'active' ? '启用' : '禁用'}\`);
  } catch (error) {
    console.error('[salon-config] 更新状态失败:', error);
    (0, response_1.error)(res, '更新状态失败', 500);
  }
});
`;

// 在 exports.default = router; 之前插入路由代码
const marker = 'exports.default = router;';
